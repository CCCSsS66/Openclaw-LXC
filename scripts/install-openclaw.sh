...
jq \
  ...
  .agents.list = (
    if (.agents.list? | type) == "array"
       and (.agents.list | length) > 0
    then
      .agents.list
    else
      [
...
read -r \
  -p "是否支持 reasoning [$DEFAULT_REASONING]，y/N: " \
  REASONING_INPUT
...
# 不把 Token 放入模板。
# 每个新 LXC 首次启动时由 openclaw-ensure-config 自动生成。
rm -f /root/.openclaw/.env
